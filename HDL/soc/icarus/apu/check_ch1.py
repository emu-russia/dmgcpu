#!/usr/bin/env python3
"""check_ch1.py - verify tb_apu_ch1 wave segments (issue #398).

Reads apu_ch1_segs.log (segment start times) + tb_apu_ch1.vcd (ch1_out
...
Prints RESULT tb_apu_ch1 PASS/FAIL for the run_all.sh suite.
"""
import re, sys

VCD, LOG = 'tb_apu_ch1.vcd', 'apu_ch1_segs.log'
...
def main():
    sig = load_vcd_sig(VCD, 'ch1_out')
    if sig is None:
        print('RESULT tb_apu_ch1 FAIL (ch1_out not in vcd)')
        return 1
    segs = seg_starts(LOG)
    ...
    for i, segn in enumerate(order):
        ...
        if not ok:
            fails += 1
    print('RESULT tb_apu_ch1 %s (%d fails)' % ('PASS' if fails == 0 else 'FAIL', fails))
